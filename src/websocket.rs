...
use crate::{
    queries::{get_admin, get_all, insert_msg, insert_user},
    ws_handler::RoomState,
    AppState,
};
...
pub async fn websocket(stream: WebSocket, state: Arc<AppState>) {
    ...
    while let Some(Ok(message)) = receiver.next().await {
        if let Message::Text(name) = message {
            if let Ok(typing_event) = serde_json::from_str::<TypingEvent>(&name) {
                let msg = format!(
                    "User {} is {} typing...",
                    username,
                    if typing_event.is_typing {
                        "now"
                    } else {
                        "no longer"
                    }
                ...
                let _ = tx.as_ref().unwrap().send(msg);
                continue;
            }
            ...
            if let Err(err) = insert_user(&state.db_pool, &connect.username, &connect.channel).await
            {
                ...
                let _ = sender
                    .send(Message::Text(String::from(
                        "Failed to insert use to database",
                    )))
                    .await;
                return;
            }
        ...
        == 1
    {
        ...
        let msg = format!(
            "--------------{} joined the {} chat created by {}",
            username, channel, admin[0].name
        );
        let _ = tx.send(msg);
    };
    ...
    let _ = sqlx::query("DELETE FROM messages WHERE channel = $1")
        .bind(channel.clone())
        .execute(&state.db_pool)
        .await;
